...
const socialIcons = {
  ...
  instagram: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 shrink-0">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0 3.675c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
    </svg>
  ),
  tiktok: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 shrink-0">
...
export default function HowToHelp() {
  ...
  const directContacts = [
    { href: 'https://t.me/thearysrms', label: 'Telegram', value: '@thearysrms', icon: telegramIcon },
    { href: 'mailto:support@scc-charity.com', label: 'Email', value: 'support@scc-charity.com', icon: emailIcon },
  ];

  const socials = [
    { href: 'https://www.facebook.com/share/18Ws4jj6ht/?mibextid=wwXIfr', label: 'Facebook', icon: socialIcons.facebook },
    { href: 'https://www.instagram.com/scc._charity', label: '@scc._charity', icon: socialIcons.instagram },
    { href: 'https://www.tiktok.com/@smileofcambodianchild', label: '@smileofcambodianchild', icon: socialIcons.tiktok },
  ];
  ...
  return (
    <section id="help" className="section-padding bg-sand">
      <div className="max-w-7xl mx-auto px-5 md:px-8">
        ...
        <ScrollReveal className="mt-10 md:mt-14 max-w-5xl mx-auto">
          <div className="glass-dark rounded-2xl p-5 md:p-6">
            <div className="grid sm:grid-cols-2 gap-4">
              {directContacts.map((contact) => (
                <a key={contact.label} href={contact.href} className="flex items-center gap-4 rounded-xl bg-white/6 px-4 py-4 text-cream hover:bg-white/10 hover:text-lime transition-colors">
                  <div className="w-10 h-10 rounded-full bg-lime-soft flex items-center justify-center shrink-0">
                    {contact.icon}
                  </div>
                  <div className="min-w-0">
                    <div className="text-xs uppercase tracking-wider text-white/35">{contact.label}</div>
                    <div className="text-sm font-medium break-words">{contact.value}</div>
                  </div>
                </a>
              ))}
            </div>
            ...
            <div className="mt-5 pt-5 border-t border-white/10">
              <p className="text-[10px] uppercase tracking-[0.16em] text-white/30 text-center mb-4">{t.donate.socialHeadline}</p>
              <div className="grid sm:grid-cols-3 gap-3">
                {socials.map((social) => (
                  <a
                    ...
                    className="flex items-center justify-center gap-2 rounded-full bg-white/5 px-4 py-3 text-sm text-cream hover:bg-white/10 hover:text-lime transition-colors"
                  >
                    {social.icon}
